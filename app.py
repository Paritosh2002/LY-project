...
def extract_info_from_format_1(file):
    ...
    for i in range(len(dic1_keys)):
        ...
        sheet.cell(column=8, row=a, value=str(item_obj_final_strip))
        hsn_obj = dic_item[2].split(":")
        hsn_obj_final = hsn_obj[-1]
        hsn_obj_final_strip = hsn_obj_final.strip()
        sheet.cell(column=9, row=a, value=str(hsn_obj_final_strip))
    ...
    if (
        ...
        sheet.cell(column=2, row=a).value == buyer_name and
        sheet.cell(column=3, row=a).value == seller_name and
        sheet.cell(column=4, row=a).value == gst_str and
        sheet.cell(column=5, row=a).value == fssai_str and
        sheet.cell(column=6, row=a).value == str(cleaned_date) and
        sheet.cell(column=7, row=a).value == float(invoice_total) and
        sheet.cell(column=8, row=a, value=str(item_obj_final_strip)) and 
        sheet.cell(column=9, row=a, value=str(hsn_obj_final_strip)) and 
        sheet.cell(column=10, row=a, value=str(weight_obj_final_strip)) and
        sheet.cell(column=11, row=a, value=str(rate_obj_final_strip)) and
        sheet.cell(column=12, row=a, value=str(amt_obj_final_strip)) 
...
def extract_info_from_format_2(file):
    ...
    if (
        ...
        sheet.cell(column=4, row=a).value == gst_str and
        sheet.cell(column=5, row=a).value == fssai_str and
        sheet.cell(column=6, row=a).value == str(date) and
        sheet.cell(column=7, row=a).value == extracted_string and
        sheet.cell(column=8, row=a, value=str(item_obj_final_strip)) and 
        sheet.cell(column=9, row=a, value=str(hsn_obj_final_strip)) and 
        sheet.cell(column=10, row=a, value=str(weight_obj_final_strip)) and
        sheet.cell(column=11, row=a, value=str(rate_obj_final_strip)) and
...
def extract_info_from_format_3(file):
    ...
    if (
        ...
        sheet.cell(column=5, row=a).value == fssai_str and
        sheet.cell(column=6, row=a).value == str(date) and
        sheet.cell(column=7, row=a).value == invoice_total and
        sheet.cell(column=8, row=a, value=str(item_obj_final_strip)) and 
        sheet.cell(column=9, row=a, value=str(hsn_obj_final_strip)) and 
        sheet.cell(column=10, row=a, value=str(weight_obj_final_strip)) and
        sheet.cell(column=11, row=a, value=str(rate_obj_final_strip)) and
